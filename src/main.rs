...
fn main() {
	...
	println!("本程序可以将WireGuard配置文件的信息转为NekoRay节点！\n");
	...
	loop {
		...
		let re = Regex::new(r"^((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)(:[0-9]+)?$").unwrap();
		loop {
			print!("输入优选IP(格式：162.159.192.1:2408)：");
			io::stdout().flush().expect("无法刷新标准输出缓冲区");
			input_endpoint.clear();
			io::stdin().read_line(&mut input_endpoint).expect("无法读取输入");
			input_endpoint = input_endpoint.trim().to_string();
			if re.is_match(&input_endpoint) {
				break;
			}
		}
		
		let ip_port: Vec<&str> = input_endpoint.split(":").collect();
		let ip = ip_port[0].trim();
		let port = ip_port[1].trim();
		...
		let node = base_str.replace("{name}", &format!("{}{}:{}", country, ip, port)).replace("{server}", ip).replace("{server_port}", port);
		...
		println!("{:-<101}", "");
		println!("节点已经生成好，将上面的NekoRay链接复制到NekoBox软件使用即可！记得要切换为sing-box核心。\n");
		println!("{:+<101}", "");
	}
}